...
`Item Code` (เฉพาะ 10 รหัสที่กำหนด — แสดงเป็น**ชื่อย่อ** ไม่ใช่รหัสเต็ม),
`Onhand`, `จัดสรร(กระจาย)`, `จัดสรร(เบิก Hub)` — 3 ค่านี้รวมเป็นบรรทัดเดียว
ในคอลัมน์ขวาของแถว (ดู `numbersLine` ใน `buildCardPayload.ts`)
...
| รหัสวัสดุ | `Item Code` | E | คอลัมน์ซ้าย — แสดงเป็น**ชื่อย่อ** เช่น `Closure 12C` (ดู `TARGET_ITEMS`) |
| ชื่อวัสดุเต็ม | `TARGET_ITEMS` | — | ตารางอธิบายท้ายการ์ด (1 บรรทัดต่อกลุ่ม ไม่ใช่ต่อรหัส) |
| Onhand | `Onhand` | I | คอลัมน์ขวา ส่วนที่ 1 ของ `numbersLine` |
| จัดสรร(กระจาย) | `จัดสรร(กระจาย)` | **K** | คอลัมน์ขวา ส่วนที่ 2 — **ชุด Optimize ลงยอดไว้ที่นี่** (ดูข้อ 4.0 เรื่องคอลัมน์เลื่อน) |
| จัดสรร(เบิก Hub) | `จัดสรร(เบิก Hub)` | **L** | คอลัมน์ขวา ส่วนที่ 3 — **ชุด MA ลงยอดไว้ที่นี่** ปัดเป็นจำนวนเต็ม |
| สถานะ | `Status` | AA | สี + น้ำหนักตัวอักษรของชื่อย่อในแถว (ไม่มี emoji แล้ว) |
| ความเสี่ยง | `Risk_Flag` | AB | ป้ายเตือนที่หัว Zone |

`Prev_Before` / `Prev_Received` ยังอ่านและคำนวณอยู่ในตรรกะ (ใช้กับการ์ดแบบอื่น
เช่น `04_card-mobile-stacked.json`) แต่**ไม่แสดงบนการ์ดหลักแล้ว**

> **หมายเหตุเรื่อง "จัดสรร(กระจาย)" กับ "จัดสรร(เบิก Hub)"**
> ชุด **Optimize** ลงยอดจ่ายไว้ที่ `จัดสรร(กระจาย)` ส่วนชุด **MA** ลงไว้ที่
> `จัดสรร(เบิก Hub)` — คนละคอลัมน์กัน การ์ดจึงต้องแสดงทั้งคู่ ไม่งั้นรอบ Optimize
> จะขึ้นศูนย์ทั้งใบ (ยืนยันจากข้อมูลจริง: Optimize(3 Aug 26) มี Onhand=0 ทุกแถว
> แต่ จัดสรร(กระจาย) รวมกว่า 30,000 ต่อ Zone)
>
> `จัดสรร(เบิก Hub)` แสดงเป็น **จำนวนเต็มเสมอ** (ฟังก์ชัน `fmtInt`) เพราะเป็น
> ยอดจ่ายจริงที่นับเป็นชิ้น ส่วน `Onhand` / `จัดสรร(กระจาย)` ยังคงทศนิยม 2 ตำแหน่ง
> เมื่อค่าต้นทางมีเศษ (ฟังก์ชัน `fmtNum` / `fmtInt`)
...
## 2. รหัสวัสดุที่รายงาน (10 รหัส แบ่ง 4 กลุ่ม)

กำหนดเป็นรายการตายตัว ไม่ได้ใช้ pattern matching
ดูที่ `TARGET_ITEMS` ใน `office-scripts/buildCardPayload.ts`
แต่ละรหัสมี `icon` / `group` / `short` / `name` / `about`
แถวข้อมูลบนการ์ดหลักแสดงเป็น **`short` ล้วน** แทน Item Code เต็ม
(ฟิลด์ `icon` ยังคำนวณไว้ให้การ์ดใบอื่นใช้ แต่การ์ดหลักไม่ได้ผูกกับมันแล้ว)

| icon (`itemIcon`) | Item Code | ชื่อย่อบนการ์ด | ชื่อวัสดุเต็ม (เก็บไว้อ้างอิง) |
|---|---|---|---|
| 🏷️ | `50MT004BB` | Name Plate Aluminium | Name Plate (Aluminium) |
| 🧰 | `52CL003BB` | Closure 12C | CLOSURE 12 C |
| 🧰 | `52CL004BB` | Closure 24C | CLOSURE 24 C |
| 🧰 | `52CL006BB` | Closure 48C | CLOSURE 48 C |
| 🧰 | `52CL009BB` | Closure 12C Inline | CLOSURE 12 C FOR OFC DROP WIRE (IN LINE) |
| 🧰 | `52CL010BB` | Closure 60C | CLOSURE 60 C |
| 🔌 | `53OF150BB` | Drop Cable 1C SC/UPC · 3m | OPTICAL FIBER DROP CABLE 1C, FLAT TYPE (G.657A) WITH 2 SC/UPC PRE-CONNECTOR, 3m. |
| 🧶 | `53OF157BB` | ARSS Fiber Cable 12C | ARSS OPTICAL FIBER CABLE 12c-FIBRE3 |
| 🧶 | `53OF158BB` | ARSS Fiber Cable 24C | ARSS OPTICAL FIBER CABLE 24c-FIBRE3 |
| 🧶 | `53OF160BB` | ARSS Fiber Cable 60C | ARSS OPTICAL FIBER CABLE 60c-FIBRE3 |

**4 กลุ่ม** — 🏷️ NAME PLATE (1 รหัส) · 🧰 CLOSURE (5 รหัส) ·
🔌 DROP CABLE (1 รหัส) · 🧶 ARSS FIBER CABLE (3 รหัส)
รหัสในกลุ่มเดียวกันใช้ icon เดียวกันเสมอ — ตารางอธิบายท้ายการ์ดจัดกลุ่มตาม
`group` แล้วถอดชื่อย่อกลับเป็น Item Code เต็มให้ (1 บรรทัดต่อกลุ่ม)
บนการ์ดหลักบรรทัดนี้ขึ้นเป็นชื่อกลุ่มล้วน เช่น `**CLOSURE** — หัวต่อ CLOSURE …`
...
| รอบ | ชุด | แถวดิบ | หลังรวม | Zone |
|---|---|---|---|---|
| MA (13 July 26) | MA | 22 | 14 | 4 |
| MA (20 July 26) | MA | 18 | 10 | 3 |
| MA (27 July 26) | MA | 10 | 7 | 3 |
| MA (3 Aug 26) | MA | 13 | 9 | 4 |
| MA (10 Aug 26) | MA | 10 | 9 | 3 |
| MA (17 Aug 26) | MA | 91 | 29 | 4 |
| Optimize(6 July 26) | Optimize | 45 | 45 | 5 |
| Optimize(3 Aug 26) | Optimize | 45 | 45 | 5 |

> ตัวเลขนี้นับด้วยชุด **10 รหัส** ปัจจุบัน (เดิม 8 รหัสจะได้น้อยกว่านี้)
...
> ⚠️ **นโยบาย emoji ของการ์ดหลัก: มีเฉพาะหัวการ์ด**
> การ์ด `01_card-main.json` แสดง emoji ตัวเดียวคือ 🏬 ที่หัวการ์ด
> ตารางข้างล่างคือค่าที่ payload ยัง**คำนวณและส่งออกมาให้ครบ** (`zoneEmoji`,
> `statusEmoji`, `itemIcon`) เพื่อให้การ์ดใบอื่น (`02_card-simple-flat.json`,
> `04_card-mobile-stacked.json`) ยังเรียกใช้ได้ — การ์ดหลักแค่ไม่ได้ผูกกับมัน

### 3.1 ตาม Data_Set (กำหนดสีหัวการ์ดทั้งใบ)

| Data_Set | emoji หัวการ์ด | container style | ป้ายบนการ์ด |
|---|---|---|---|
| `MA` | 🏬 | `accent` (ฟ้า) | MA — Weekly Allocation |
| `Optimize` | 🏬 | `good` (เขียว) | Optimize — Allocation Plan |

ทั้งสองชุดใช้ 🏬 (Store) เหมือนกัน — แยกกันด้วย**สีของการ์ด**และ**ป้ายใต้ชื่อ**

### 3.2 ตาม Zone (หัวกลุ่มตาราง)

emoji ในคอลัมน์นี้ **ไม่ได้แสดงบนการ์ดหลักแล้ว** (อยู่ในฟิลด์ `zoneEmoji`
ให้การ์ดใบอื่นใช้) หัวกลุ่ม Zone บนการ์ดหลักแยกด้วยตัวหนา + สี + เส้นคั่นแทน

| Zone | emoji (`zoneEmoji`) | จังหวัด |
|---|---|---|
...
> การ์ดหลักไม่แสดง emoji สถานะแล้ว — สถานะสื่อผ่าน **สีและตัวหนา** ของชื่อย่อ
> ในแถวแทน (`statusColor` / `rowWeight`) และผ่านกล่องสรุป 4 ช่องด้านบน
> ฟิลด์ `statusEmoji` / `status` ยังอยู่ใน payload ให้การ์ดแบบอื่นเรียกใช้ได้

### 3.4 ตาม Risk_Flag

| Risk_Flag | ผลบนการ์ด |
|---|---|
| `เสี่ยงขาด` | แถวเป็นตัวหนา + ขึ้นบนสุดของ Zone + นับในกล่อง "เสี่ยงขาด" + ป้ายแดงที่หัว Zone |
...
| ค่า | วิธีรวม |
|---|---|
| `Onhand` | บวกทุกจังหวัดในโซน |
| `จัดสรร(กระจาย)` | บวกทุกจังหวัดในโซน |
| `จัดสรร(เบิก Hub)` | บวกทุกจังหวัดในโซน |
| `Prev_Before` / `Prev_Received` | บวกทุกจังหวัดในโซน (คำนวณไว้ ไม่แสดงบนการ์ดหลักแล้ว) |
...
| # | ชีต | กระทบอะไร | สถานะ |
|---|---|---|---|
| 4.0 | `MA&Optimize NER` (ชีตต้นทาง) | `จัดสรร(เบิก Hub)` ได้ค่าของ `จัดสรร` | ✅ สคริปต์ตรวจและแก้เองแล้ว |
...
มีคอลัมน์ว่างที่ไม่มีหัวตารางแทรกอยู่หลัง `Onhand`
...
| `จัดสรร(กระจาย)` | คอลัมน์ถัดไป ➡️ |
| `จัดสรร(เบิก Hub)` | คอลัมน์ถัดไป ➡️ |
...
**ผลกระทบ:** สคริปต์รุ่นก่อนแสดงค่าของ `จัดสรร(กระจาย)` ในช่อง `จัดสรร(เบิก Hub)`
ผิด **179 จาก 217 แถว** ที่เข้าเกณฑ์ (นับด้วยชุด 8 รหัสเดิม — ยืนยันซ้ำกับชุด
10 รหัสปัจจุบันแล้วว่ากฎยังเป็นจริง 2,365/2,365 แถวเหมือนเดิม)
...
> และ `Net_Used = Expected_Before − Onhand`
...
| K | `Alloc` | จัดสรร(เบิก Hub) | **จัดสรร(เบิก Hub) หาไม่เจอ** |
