...
`Item Code` (เฉพาะ 8 รหัสที่กำหนด), `Onhand`, `Prev_Period`,
`Prev_Before`, `Prev_Received` และ *จัดสรร(เบิก Hub) = Distribution period ในรอบนั้น ๆ*
...
| Onhand | `Onhand` | I | คอลัมน์ที่ 2 |
| จัดสรร(เบิก Hub) | `จัดสรร(เบิก Hub)` | **L** | คอลัมน์ที่ 3 — **ปัดเป็นจำนวนเต็ม** (ดูข้อ 4.0 เรื่องคอลัมน์เลื่อน) |
...
> **หมายเหตุเรื่อง "จัดสรร(เบิก Hub)"**
> ตามโจทย์ ค่าในคอลัมน์นี้คือยอดจ่ายของรอบ (Distribution) ในรอบนั้น ๆ
> การ์ดจึงตั้งหัวคอลัมน์ว่า **"จัดสรร(เบิก Hub)"** และผูกกับรอบที่ระบุในหัวการ์ด
>
> ค่านี้แสดงเป็น **จำนวนเต็มเสมอ** (ฟังก์ชัน `fmtInt`) เพราะเป็นยอดจ่ายจริงที่นับเป็นชิ้น
> ต่างจาก `Onhand` / `Prev_Before` / `Prev_Received` ที่ยังคงทศนิยม 2 ตำแหน่ง
...
**ผลกระทบ:** สคริปต์รุ่นก่อนแสดงค่าของ `จัดสรร` ในช่อง `จัดสรร(เบิก Hub)`
