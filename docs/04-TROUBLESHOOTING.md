...
เวอร์ชันล่าสุดแก้ 2 อย่าง:

1. เปลี่ยนไปวัดขนาดแบบ **แย่ที่สุดที่เป็นไปได้** (`wireBytes`) แทนการวัด UTF-8 ตรง ๆ
2. ลดงบจาก 26,000 เหลือ **20,000 ไบต์** เหลือที่ให้ซองข้อความราว 8 KB

**ถ้าวางสคริปต์ใหม่แล้วยังเจออีก** ให้ลดเลขบรรทัดนี้ลงทีละ 2,000

```ts
const CARD_SAFE_BYTES = 20000;      // ← อยู่ราวบรรทัดที่ 1,243 ของสคริปต์
```

**ทางลัดที่ไม่ต้องแก้สคริปต์:** ลดค่า `topRows` ใน action `Run script`
จาก `40` เป็น `20` แล้วลองใหม่ — ถ้าผ่าน แปลว่าเป็นปัญหาขนาดจริง

**ขนาดหลังแก้ (วัดจากข้อมูลในไฟล์นี้)**

| รอบ | UTF-8 | worst case | เทียบเพดาน 28 KB |
|---|---|---|---|
| Optimize (3 Aug 26) — 5 Zone | ~17.8 KB | 19.1 KB | ✅ เหลือที่ 8.9 KB |
| MA (17 Aug 26) — 4 Zone | ~18.3 KB | 19.5 KB | ✅ เหลือที่ 8.5 KB |
| MA (10 Aug 26) — 3 Zone | 10.1 KB | 11.0 KB | ✅ เหลือที่ 17 KB |

แถวที่เกินงบจะถูกตัด แล้วสรุปเป็นบรรทัด *"…และอีก N รายการ"* พร้อมปุ่มไป Dashboard
การ์ดรวมแถวซ้ำให้แล้ว (1 Zone + 1 Item Code = 1 แถว) เช่น MA (17 Aug 26)
มี 74 แถวดิบ เหลือ 24 แถวหลังรวม
