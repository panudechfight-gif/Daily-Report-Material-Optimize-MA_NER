...
## 7.1 ตัวเลข "เบิกจาก Hub" บนการ์ดไม่ตรงกับที่เปิดดูในชีต

**ถ้าใช้สคริปต์เวอร์ชันตั้งแต่ 16 ส.ค. 2026 เป็นต้นไป เรื่องนี้แก้แล้ว** — อ่านไว้เพื่อเข้าใจที่มา

ชีต `MA&Optimize NER` มี **คอลัมน์ว่างที่ไม่มีหัวตาราง** แทรกอยู่หลัง `OMC-Onhand`
ข้อมูลของบล็อกนี้จึงไปอยู่ทางขวาของหัวตารางตัวเองอยู่ 1 ช่อง

| หัวตารางเขียนว่า | ข้อมูลจริงอยู่ที่ |
|---|---|
| `จัดสรร` | คอลัมน์ถัดไป |
| `เบิกจาก Hub` | คอลัมน์ถัดไป |
| `จัดสรรเพิ่ม` | คอลัมน์ถัดไป |
| `รวม 3 รายการ` | คอลัมน์ถัดไป |
| `เหตุผล` | ไม่มีข้อมูล (เป็นหัวตารางที่เหลือค้าง) |

สคริปต์รุ่นก่อนอ่านตามชื่อหัวตารางตรง ๆ ช่อง "เบิกจาก Hub" บนการ์ดจึงได้ค่าของ **`จัดสรร`** แทน
กระทบ **179 จาก 217 แถว** ที่เข้าเกณฑ์

**คอลัมน์ที่ไม่โดนผลกระทบ:** `OMC-Onhand`, `Prev_Before`, `Prev_Received` — ตรวจแล้วตรงตำแหน่ง
