...
```
   ...
   🎨 ประกอบเป็น Adaptive Card
      (สีตาม Data_Set และ Status — emoji มีเฉพาะหัวการ์ด)
            │
            ▼
   💬 โพสต์เข้าห้อง Teams
      พร้อมปุ่ม Dashboard และเปิดไฟล์ต้นทาง
```
...
| `reportType` | เว้นว่าง | ไม่ใช้ในรอบนี้ — คงไว้ให้ Flow เดิมที่ตั้งค่าไว้แล้วยังใช้ได้ |

> ✅ **ทุกช่องเว้นว่างได้ ไม่มีดอกจันบังคับกรอกแล้ว**
>
> สคริปต์ประกาศพารามิเตอร์เป็นแบบไม่บังคับ (`period?: string`) ทั้งหมด
> Power Automate จึงไม่ขึ้น `*` และไม่ขึ้น error นี้อีก
> `Invalid parameter for 'Run script'. Error: 'ScriptParameters/period' is required.`
>
> **ถ้ายังเห็นดอกจันหรือยังขึ้น error อยู่** แปลว่า Flow ยังจำรายการพารามิเตอร์ชุดเก่าไว้
> ให้ **ลบ action `Run script` แล้วเพิ่มใหม่** (หรือเลือกไฟล์/สคริปต์ซ้ำอีกครั้ง)
> Power Automate จะอ่านลายเซ็นใหม่ของสคริปต์ให้เอง — ดูข้อ 2.1 ใน
> [04-TROUBLESHOOTING.md](04-TROUBLESHOOTING.md)
>
> ค่าที่ใส่ได้ในช่อง `period`:
>
> | ใส่ | ได้อะไร |
> |---|---|
> | เว้นว่าง หรือ `-` `auto` `latest` `null` | รอบ **Optimize** ล่าสุด (รายงานหลัก) |
...
> แต่ชื่อรอบต้องตรงเป๊ะรวมวงเล็บและช่องว่าง (เว้นวรรคหน้า-หลังตัดให้เอง)
...
> |---|---|---|---|---|
> | Optimize (3 Aug 26) | 5 | **45 ครบ** | 18.5 KB | 20.8 KB ✅ |
> | MA (17 Aug 26) | 4 | **29 ครบ** | 14.4 KB | 16.3 KB ✅ |
>
...
- [ ] แถบตัวเลข 4 กล่อง: เสี่ยงขาด, พอใช้, ปกติ, รวม
- [ ] **หัวกลุ่มขึ้นครบทุก Zone** พร้อม emoji
      (RC2-NMA, RC2-UBN, RC3-KKN, RC3-UDN, RC3-SNK — เท่าที่รอบนั้นมีข้อมูล)
      หัวกลุ่มเป็นตัวหนา + มีเส้นคั่น ไม่มี emoji แล้ว
- [ ] แถวข้อมูล 2 คอลัมน์: **ชื่อย่อ** (เช่น `Closure 12C`) กับ
...
- [ ] ตารางอธิบายท้ายการ์ดมี 4 กลุ่ม: NAME PLATE · CLOSURE ·
...
| | Optimize — รอบเดือน | MA — รอบสัปดาห์ |
|---|---|---|
